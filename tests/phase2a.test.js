...
test('Phase2AのPIN、HMAC、重複防止、LINE打刻、訂正履歴が動作する', async () => {
  ...
  try {
    ...
    const blocked=await signed('event-second',{action:'start',line_user_id:'U-test',menu_code:'chips',occurred_at:'2026-07-19T09:10:00+09:00'});assert.equal(blocked.status,200);assert.equal((await blocked.json()).data.state,'working');
    ...
    const workDuringBreak=await signed('event-work-during-break',{action:'start',line_user_id:'U-test',menu_code:'chips',occurred_at:'2026-07-19T11:05:00+09:00'});assert.equal(workDuringBreak.status,200);const breakState=(await workDuringBreak.json()).data;assert.equal(breakState.state,'break');assert.match(breakState.message,/休憩を終了/);
