...
`POST /masters/employees`と`PATCH /masters/employees/{id}`は、`code`, `name`, `department_id`, `hourly_rate`, `line_enabled`, `employment_status`を一括で扱う。時給は`labor_cost_rates`へ有効期間付きで保存し、従業員本体と同一取引で更新する。退職またはLINE対象外への変更時は有効なLINE紐付けを解除する。
